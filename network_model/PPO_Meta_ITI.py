...
class cue_input(): 
    ...
    def step(self,a):        
        ...
        if self.time_step >= self.trial_len-1:
            # if self.reward_to_be>=1 and self.food_picking == False:
            #     reg = reward_to_be
            done_trial = True
...
class AC_Network():
    def __init__(self,np_seed,beta_e,a_size,scope,trainer):
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            self.num_LSTMunits_DMS=32
            self.num_LSTMunits_DLS=32
            ...
            with tf.compat.v1.variable_scope('DMS'):
                c_init_DMS = np.zeros((1, self.num_LSTMunits_DMS), np.float32)
                h_init_DMS = np.zeros((1, self.num_LSTMunits_DMS), np.float32)
                self.state_init_DMS = [c_init_DMS, h_init_DMS]            
                c_in_DMS = tf.compat.v1.placeholder(tf.float32, [1, self.num_LSTMunits_DMS])
                h_in_DMS = tf.compat.v1.placeholder(tf.float32, [1, self.num_LSTMunits_DMS])
                self.state_in_DMS = [c_in_DMS,h_in_DMS]
                c_in_batch_DMS=tf.tile(c_in_DMS,[self.batch_size,1]) #shape (batch_size,lstm_cell.state_size.c)
                h_in_batch_DMS=tf.tile(h_in_DMS,[self.batch_size,1]) 
                self.state_in_batch_DMS = [c_in_batch_DMS,h_in_batch_DMS] 
                state_in_DMS = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c_in_batch_DMS, h_in_batch_DMS)           
                self.lstm_cell_DMS = tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_LSTMunits_DMS,state_is_tuple=True) 
                lstm_outputs_DMS, lstm_state_DMS = tf.compat.v1.nn.dynamic_rnn(
                self.lstm_cell_DMS, inputs, initial_state=state_in_DMS, sequence_length=self.seq_length, time_major=False)
                lstm_c_DMS, lstm_h_DMS = lstm_state_DMS
                self.state_out_DMS = [lstm_c_DMS, lstm_h_DMS] #this output will be feed in later, so should be the same shape as the self.state_in
                self.rnn_out_DMS   = lstm_outputs_DMS 
                lstm_outputs_DMS_flatten=tf.reshape(lstm_outputs_DMS, [-1, self.num_LSTMunits_DMS])
            
            with tf.compat.v1.variable_scope('DLS'):
                c_init_DLS = np.zeros((1, self.num_LSTMunits_DLS), np.float32)
                h_init_DLS = np.zeros((1, self.num_LSTMunits_DLS), np.float32)            
                self.state_init_DLS = [c_init_DLS, h_init_DLS]            
                c_in_DLS = tf.compat.v1.placeholder(tf.float32, [1, self.num_LSTMunits_DLS])
                h_in_DLS = tf.compat.v1.placeholder(tf.float32, [1, self.num_LSTMunits_DLS])
                self.state_in_DLS = [c_in_DLS,h_in_DLS]
                c_in_batch_DLS=tf.tile(c_in_DLS,[self.batch_size,1]) #shape (batch_size,lstm_cell.state_size.c)
                h_in_batch_DLS=tf.tile(h_in_DLS,[self.batch_size,1]) 
                self.state_in_batch_DLS = [c_in_batch_DLS,h_in_batch_DLS] 
                state_in_DLS = tf.compat.v1.nn.rnn_cell.LSTMStateTuple(c_in_batch_DLS, h_in_batch_DLS)           
                self.lstm_cell_DLS = tf.compat.v1.nn.rnn_cell.LSTMCell(self.num_LSTMunits_DLS,state_is_tuple=True)             
                lstm_outputs_DLS, lstm_state_DLS = tf.compat.v1.nn.dynamic_rnn(
                self.lstm_cell_DLS, inputs, initial_state=state_in_DLS, sequence_length=self.seq_length, time_major=False)
                lstm_c_DLS, lstm_h_DLS = lstm_state_DLS
                self.state_out_DLS = [lstm_c_DLS, lstm_h_DLS] #this output will be feed in later, so should be the same shape as the self.state_in
                self.rnn_out_DLS   = lstm_outputs_DLS 
                lstm_outputs_DLS_flatten=tf.reshape(lstm_outputs_DLS, [-1, self.num_LSTMunits_DLS])
            
            lstm_outputs = tf.concat([lstm_outputs_VS_flatten,lstm_outputs_DMS_flatten, lstm_outputs_DLS_flatten], axis=1)
            ...
            self.value_DMS = tf.keras.layers.Dense(1, 
                activation=None,
                kernel_initializer=normalized_columns_initializer(1.0))(lstm_outputs_DMS_flatten)
            self.value_DLS = tf.keras.layers.Dense(1, 
                activation=None,
                kernel_initializer=normalized_columns_initializer(1.0))(lstm_outputs_DLS_flatten)
            ...
            if scope != 'global':
                ...
                self.target_v_DMS = tf.compat.v1.placeholder(shape=[None],dtype=tf.float32)
                self.target_v_DLS = tf.compat.v1.placeholder(shape=[None],dtype=tf.float32)
                ...
                self.value_loss_DMS = 0.5 * tf.reduce_sum(tf.square(self.target_v_DMS - tf.squeeze(self.value_DMS,1)))
                self.value_loss_DLS = 0.5 * tf.reduce_sum(tf.square(self.target_v_DLS - tf.squeeze(self.value_DLS,1)))
                self.value_loss = (self.value_loss_VS + self.value_loss_DMS + self.value_loss_DLS)/tf.cast(self.seq_length, tf.float32)
    ...
    def perform_trial(self,sess,obs,env,rnn_state_init_VS,
                      rnn_state_init_DMS,rnn_state_init_DLS):
        trial_buffer = []
        trial_state_VS   = []
        trial_state_DMS = []
        trial_state_DLS = []
        ...
        while done_trial==False: 
            a_dist,v_VS, v_DMS, v_DLS, rnn_state_VS,rnn_output_VS, \
            rnn_state_DMS,rnn_output_DMS, rnn_state_DLS,rnn_output_DLS = \
            sess.run([self.policy,self.value_VS,self.value_DMS,self.value_DLS,
                      self.state_out_VS,self.rnn_out_VS, 
                      self.state_out_DMS,self.rnn_out_DMS, 
                      self.state_out_DLS,self.rnn_out_DLS], 
                feed_dict={
                self.batch_size: 1,
                self.seq_length: [1],
                self.obs:[[obs]],
                ...
                self.state_in_DMS[0]: rnn_state_init_DMS[0],
                self.state_in_DMS[1]: rnn_state_init_DMS[1],
                self.state_in_DLS[0]: rnn_state_init_DLS[0],
                self.state_in_DLS[1]: rnn_state_init_DLS[1]})    
            ...
            trial_buffer_temp.extend([v_VS[0,0],v_DMS[0,0],v_DLS[0,0],pi_a])
            trial_buffer.append(trial_buffer_temp) 
            trial_state_VS.append(rnn_state_VS)
            trial_state_DMS.append(rnn_state_DMS)
            trial_state_DLS.append(rnn_state_DLS)
            rnn_state_init_VS  = rnn_state_VS #within a trial, continous training
            rnn_state_init_DMS = rnn_state_DMS
            rnn_state_init_DLS = rnn_state_DLS
            obs = next_obs
        ...
        return obs,time_step_food,trial_len,trial_reward,trial_buffer,\
               trial_state_VS,trial_state_DMS,trial_state_DLS,\
               rnn_state_VS,rnn_state_DMS,rnn_state_DLS,v_VS[0,0],v_DMS[0,0],v_DLS[0,0] 

class Worker():
    def __init__(self,np_seed,env,name,a_size,batch_size,n_step,
        beta_e,gamma_VS,gamma_DMS,gamma_DLS,trainer,model_path,global_steps):
        self.seed = np_seed
        self.env  = env
        ...
        self.gamma_DMS    = gamma_DMS
        self.gamma_DLS    = gamma_DLS
    ...
    def train(self,sess,rollout,batch_rnn_state_VS,batch_rnn_state_DMS,batch_rnn_state_DLS):
        ...
        batch_rnn_state0_DMS=np.squeeze(np.array(batch_rnn_state_DMS[0]),axis=1)
        batch_rnn_state1_DMS=np.squeeze(np.array(batch_rnn_state_DMS[1]),axis=1)
        batch_rnn_state0_DLS=np.squeeze(np.array(batch_rnn_state_DLS[0]),axis=1)
        batch_rnn_state1_DLS=np.squeeze(np.array(batch_rnn_state_DLS[1]),axis=1)
        
        feature_end = 2+feature_dim+background_dim

        rollout     = np.array([trial[:self.time_horizon+self.n_step+1] for trial in rollout])
        rewards     = rollout[:,:,1]
        values_VS   = rollout[:,:,feature_end+1]
        values_DMS  = rollout[:,:,feature_end+2]
        values_DLS  = rollout[:,:,feature_end+3]
        rollout     = rollout[:,:self.time_horizon,:]
        actions     = rollout[:,:,0]        
        obs         = rollout[:,:,2:feature_end+1]
        pi_as       = rollout[:,:,feature_end+4]
        
        discounted_rewards_VS =np.zeros_like(actions)
        discounted_rewards_DMS=np.zeros_like(actions)
        discounted_rewards_DLS=np.zeros_like(actions)
        advantages_VS =np.zeros_like(actions)
        advantages_DMS=np.zeros_like(actions)
        advantages_DLS=np.zeros_like(actions)
        advantages    =np.zeros_like(actions)
        for k in range(self.batch_size): 
            
            discounted_rewards_VS[k] = discount_nstep(rewards[k],self.gamma_VS,\
                                       self.time_horizon, self.n_step) \
                                     + np.power(self.gamma_VS,self.n_step) * values_VS[k,self.n_step:self.time_horizon+self.n_step]
            discounted_rewards_DMS[k]= discount_nstep(rewards[k],self.gamma_DMS,\
                                       self.time_horizon, self.n_step) \
                                     + np.power(self.gamma_DMS,self.n_step) * values_DMS[k,self.n_step:self.time_horizon+self.n_step]
            discounted_rewards_DLS[k]= discount_nstep(rewards[k],self.gamma_DLS,\
                                       self.time_horizon, self.n_step) \
                                     + np.power(self.gamma_DLS,self.n_step) * values_DLS[k,self.n_step:self.time_horizon+self.n_step]

            # for VS
            advantages_VS[k] = rewards[k,:self.time_horizon] + \
            ...
            # for DMS
            advantages_DMS[k] = rewards[k,:self.time_horizon] + \
                               self.gamma_DMS * values_DMS[k,1:self.time_horizon+1] - \
                               values_DMS[k,:self.time_horizon] 
            advantages_DMS[k] = discount(advantages_DMS[k],lambda_gae)
            # for DLS
            advantages_DLS[k] = rewards[k,:self.time_horizon] + \
                               self.gamma_DLS * values_DLS[k,1:self.time_horizon+1] - \
                               values_DLS[k,:self.time_horizon] 
            advantages_DLS[k] = discount(advantages_DLS[k],lambda_gae)
            
            advantages[k] = (advantages_VS[k]+advantages_DMS[k]+advantages_DLS[k])/3.0
        ...
        feed_dict = {self.local_AC.batch_size: self.batch_size,
            ...
            self.local_AC.target_v_VS:discounted_rewards_VS.flatten(),            
            self.local_AC.target_v_DMS:discounted_rewards_DMS.flatten(),            
            self.local_AC.target_v_DLS:discounted_rewards_DLS.flatten(),            
            self.local_AC.advantages:advantages.flatten(),
            ...
            self.local_AC.state_in_batch_DMS[0]: batch_rnn_state0_DMS,
            self.local_AC.state_in_batch_DMS[1]: batch_rnn_state1_DMS,
            self.local_AC.state_in_batch_DLS[0]: batch_rnn_state0_DLS,
            self.local_AC.state_in_batch_DLS[1]: batch_rnn_state1_DLS}
        v_l,p_l,e_l,g_n,v_n,_ = sess.run([self.local_AC.value_loss,
            ...
            feed_dict=feed_dict)
        return v_l/self.batch_size, p_l/self.batch_size, e_l/self.batch_size, g_n,v_n
   
    def perform_batch(self,sess,obs,rnn_state_init_VS,rnn_state_init_DMS,rnn_state_init_DLS):
        trial_lengths = []
        trial_rewards = []
        trial_values_VS  = []
        trial_values_DMS = []
        ...
        batch_rnn_state1_VS =[]
        batch_rnn_state_DMS =[]
        batch_rnn_state0_DMS=[]
        batch_rnn_state1_DMS=[]
        batch_rnn_state_DLS =[]
        batch_rnn_state0_DLS=[]
        batch_rnn_state1_DLS=[]
        done_batch     = False
        batch_count    = 0
        ...
        while done_batch == False:
            ...
            batch_rnn_state0_DMS.append(rnn_state_init_DMS[0])
            batch_rnn_state1_DMS.append(rnn_state_init_DMS[1])
            batch_rnn_state0_DLS.append(rnn_state_init_DLS[0])
            batch_rnn_state1_DLS.append(rnn_state_init_DLS[1])

            # to collect data for one time horizon
            step = 0
            while step <= self.time_horizon + self.n_step:                           
                obs,_,trial_length,trial_reward,trial_buffer,\
                _,_,_,rnn_state_VS,rnn_state_DMS,rnn_state_DLS,value_VS,value_DMS,value_DLS \
                = self.local_AC.perform_trial(sess,obs,self.env,\
                    rnn_state_init_VS,rnn_state_init_DMS,rnn_state_init_DLS)                
                ...
                rnn_state_init_VS  = rnn_state_VS # continous training.
                rnn_state_init_DMS = rnn_state_DMS
                rnn_state_init_DLS = rnn_state_DLS
        ...
        batch_rnn_state_DMS=[batch_rnn_state0_DMS,batch_rnn_state1_DLS]
        batch_rnn_state_DLS=[batch_rnn_state0_DMS,batch_rnn_state1_DLS]
        v_l,p_l,e_l,g_n,v_n = self.train(sess,batch_buffer,batch_rnn_state_VS,\
            batch_rnn_state_DMS,batch_rnn_state_DLS)
        return obs,trial_lengths,rnn_state_VS,rnn_state_DMS,rnn_state_DLS,v_l,p_l,e_l,g_n,v_n
    def work(self,sess,coord,saver,train,traininfofile): 
        ...
        with sess.as_default(), sess.graph.as_default(): 
            ...
            rnn_state_DMS = self.local_AC.state_init_DMS
            rnn_state_DLS = self.local_AC.state_init_DLS           
            obs = self.env.reset()                       
            with open(traininfofile, 'w') as f:
                while not coord.should_stop():

                    sess.run(self.update_local_ops)   
                    rnn_state_init_VS = rnn_state_VS #the rnn state at the last trial for a batch is used 
                                                     #for the initial input for next batch.                                    
                    rnn_state_init_DMS = rnn_state_DMS
                    rnn_state_init_DLS = rnn_state_DLS
                    obs,_,rnn_state_VS,rnn_state_DMS,rnn_state_DLS,v_l,p_l,e_l,g_n,v_n = \
                    self.perform_batch(sess,obs,rnn_state_init_VS,rnn_state_init_DMS,rnn_state_init_DLS)
...
if __name__ == '__main__':
    ...
    with tf.device("/cpu:0"): 
        ...
        for i in range(num_workers):
            workers.append(Worker(np_seed,cue_input(np_seed),i,a_size,batch_size,n_step,
                beta_e,gamma_VS,gamma_DMS,gamma_DLS,trainer,model_path,global_steps))
