    ...
    for train_num in train_nums:
        ...
        with open(evainfofile, 'a') as f:
            if train_num <= train_nums[0]:
                f.write("seed: %d\n" % seed)
            f.write("eva Model: %d\n" % train_num)
        path = os.path.join(os.getcwd(),"model_meta","checkpoint")
        ...
        with tf.compat.v1.Session() as sess:
            ...
            rnn_state_init_DMS = master_AC.state_init_DMS
            rnn_state_init_DLS = master_AC.state_init_DLS
            ...
            while done==False: 
                ...
                _,_,_,rnn_state_VS,rnn_state_DMS,rnn_state_DLS,\
                value_VS,value_DMS,value_DLS\
                = master_AC.perform_trial(sess,obs,env,rnn_state_init_VS,rnn_state_init_DMS,
                                          rnn_state_init_DLS)                
        ...
        # if with_units_data:
        #     data_model_units = {'episode_states_VS': episode_states_VS, 'episode_states_DMS': episode_states_DMS,\
        #                         'episode_states_DLS': episode_states_DLS}
        #     filename1 = 'RLmodel'+'_data_units_'+str(train_num)+'_eva_'+str(eva_num)+'trials_'+\
        #                'gamma_'+str(gamma_VS)+'_'+str(gamma_DMS)+'_'+str(gamma_DLS)+'_dt100_betae'+str(beta_e)+'_seed_'+str(seed)+'.npz'
